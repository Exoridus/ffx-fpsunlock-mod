...
public unsafe sealed partial class FpsUnlockModule
{
    ...
    ///
    ///     The two are not interchangeable. <see cref="FpsUnlockConfig.SyncDataAware"/> decides only
    ...
    private void h_yi_anim_info_init(nint info)
    {
        ...
        _logger.Info("[FpsUnlock] Event worker clock forced to the vertical blank counter.");
    }
    ...
    private void h_camera_move_acc(uint camera_id, uint mode_non_ref, uint mode_polar, uint a4, uint a5, uint a6, uint a7)
    {
        ...
        if (_config.SurveyCamera && _camera_acc_calls <= 8)
            _logger.Info($"[FpsUnlock] MsCameraMoveAcc #{_camera_acc_calls}: cam={camera_id} p2={mode_non_ref} " +
     ...
     * clamped integer, so the correction is the hold in FpsUnlockModule.CrossFade and scaling here as
     ...
     * leaves nothing behind. See FpsUnlockModule.MotionSurvey. */
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
