...
public unsafe sealed partial class FpsUnlockModule
{
    ...
    private void init_overlay_spawn_gate()
    {
        ...
        if (current != expected)
        {
            _logger.Error($"[FpsUnlock] gMagicFunctions slot 764 at RVA 0x{EngineAddresses.MagicFunctionsSgCountSlot:X} " +
        ...
        _logger.Info($"[FpsUnlock] Overlay spawn gate clock installed at 0x{counter:X8}; gMagicFunctions slot 764 " +
