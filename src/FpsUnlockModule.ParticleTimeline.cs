...
public unsafe sealed partial class FpsUnlockModule
{
    ...
    private void patch_particle_timeline()
    {
        ...
        {
            _logger.Info("[FpsUnlock] Particle timeline left at its authored rate; the target framerate needs no change.");
            return;
        }
        ...
        foreach ((string name, nint rva) in AgeStepSites)
        {
            ...
            if (!matches)
            {
                _logger.Error($"[FpsUnlock] Particle timeline site {name} at RVA 0x{rva:X} does not carry " +
