...
public unsafe sealed partial class FpsUnlockModule
{
    ...
    private void patch_dream_overlay_stars()
    {
        ...
        if (!matches)
        {
            _logger.Error($"[FpsUnlock] Dream overlay star counter at RVA 0x{EngineAddresses.DreamStarCounterCompare:X} " +
        ...
        {
            _logger.Info("[FpsUnlock] Dream overlay star counter left at 3; the target framerate needs no change.");
            return;
        }

        _patches.Write(EngineAddresses.DreamStarCounterCompare + DreamStarThresholdOffset, [(byte)threshold]);
        _logger.Info($"[FpsUnlock] Dream overlay star counter set to {threshold} presented frames per pass.");
    }
}
